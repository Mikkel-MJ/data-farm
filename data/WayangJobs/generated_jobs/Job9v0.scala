...
val filter1 = source0.filter(x=>x._11.getTime <= dateFormatter.parseDateTime("1995-06-19").getMillis)
...
val map3 = source4.map(x=>x)
                

val join2 = filter1.keyBy[Long](_._3).join(map3.keyBy[Long](_._2)).map(x => x.field1)
